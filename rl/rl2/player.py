...
class PlayerControllerRL(PlayerController, FishesModelling):
    ...
    def q_learning(self):
        ...
        Q = np.array([[np.random.rand() for _ in range(0, na)] for _ in range(0, ns)])
        # ADD YOUR CODE SNIPPET BETWEEN EX. 2.1
        ...
        while episode <= self.episode_max and diff > self.threshold:
            ...
            while not end_episode:
                ...
                Q[s_current, action] = Q[s_current, action] + lr * (R + discount * np.nanmax(Q[s_next]) - Q[s_current, action]) # * (1 - lr) + lr * (R + discount * next_action)
                # ADD YOUR CODE SNIPPET BETWEEN EX. 2.2
...
class PlayerControllerRandom(PlayerController):
    ...
    def random_agent(self):
        ...
        action_taken_counts = np.array([[0 for _ in range(0, na)] for _ in range(0, ns)])
        # ADD YOUR CODE SNIPPET BETWEEN EX. 1.2
        ...
        while episode <= self.episode_max:
            ...
            while not end_episode:
                ...
                action = np.random.choice(possible_actions) # choose a random action
                action_taken_counts[s_current][action] += 1 # increment the count of that action in current state
                # ADD YOUR CODE SNIPPET BETWEEN EX. 1.2
        ...
        for i in range(0, ns):
            for j in range(0, na): 
                if action_taken_counts[i][j] == max(action_taken_counts[i]):
                    n.append(j)
        #n = [max(action_taken_counts[i]) for i in range(0, ns)] # list of count of most chosen action in each state
        return n
